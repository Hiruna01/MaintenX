...
///
/// GET /api/analytics/verification. Every check in every state, and no date range — the
/// verification component's own health (is the sweep keeping up, are reporters answering).
/// The estate-wide reopen rate with its category split and trend is MetricsDto's, at
/// GET /api/analytics/metrics, and both rates use the same MetricRules.Percent over the
/// same answered-only denominator.
/// </summary>
